...
There is no build system or package manifest — these are loose Python files that MO2 loads at runtime by copying them into its `plugins/` directory. There *is* a pytest suite (`tests/`, config in `pyproject.toml`, CI in `.github/workflows/test.yml`) that runs against a hand-written `mobase` mock.
...
- **Settings**: `enabled`, `prefer_fomod`, `priority` (default 120), `force_dialog`, per-recognizer `recognizer.<name>.enabled` toggles, `use_rootbuilder` (`auto` | `always` | `never`, default `auto`), plus per-game `palworld_platform` / `palworld_server_platform` (`steam` | `xbox`, default `steam`; legacy value `gamepass` is silently normalized to `xbox` with a deprecation warning).
...
- **Layout rewrite**: `.pak` to `Content/Paks/{ROOT|~mods|Mods|LogicMods|<custom>}/` (per-file user choice); `main.lua` to `Binaries/Win64/ue4ss/Mods/<modname>/` (or `Binaries/WinGDK/ue4ss/Mods/<modname>/` when platform is `xbox`); `.json` to `Content/Paks/LogicMods/`; everything else at archive root removed. The `ue4ss/` path segment is **mandatory** — UE4SS only loads mods under `Binaries/<runtime>/ue4ss/Mods/`, and omitting it silently prevents every script mod from loading. This path lives in exactly one place in code: `ue4ss_mods_base(platform)` in `models.py` — do not re-derive it inline.
- **Root Builder routing**: when Root Builder is detected (or `use_rootbuilder=always`), **UE4SS C++ DLL plugins only** — the `ue4ss_plugin` and `dll_plugin` recognizers — are routed to `Root/Pal/` + `ue4ss_mods_base(platform)` via `rootbuilder_ue4ss_mods_base()`, which *composes onto* `ue4ss_mods_base()` rather than re-deriving it. The `Pal/` segment is the game-root offset: MO2's data root is `<install>/Pal/` (`GameDataPath = "Pal"`), but Root Builder deploys from the install root. Lua scripts, PalSchema content, paks, and json deliberately stay on the plain path — they load fine through USVFS. A recognizer that routes into an unknown top-level folder must declare it in `DiscoveryResult.extra_root_names`, or the root cleanup pass deletes it.
...
- **Tests**: `pytest tests/ -v --tb=short` from the repo root. `tests/mobase_mock.py` stands in for `mobase` and `tests/conftest.py` stubs PyQt6, so the suite runs on macOS. There is no build or lint runner. Final validation still means copying files into a real MO2 install on Windows and exercising the installer with sample archives — anything Windows-only (Root Builder detection, the WinGDK path) cannot be settled by the suite alone.
